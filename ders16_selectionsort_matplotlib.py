"""
   1. selection sort 
    'kucukten buyuğe sıralamak istiyorsak if içinde < mü diye kontrol ediyoruz.'
   2. matplotlib kütüphanesi kullanmayı öğrendik 
        > random değer atadık
        > iki nokta verildi ve distance hesapladık
        > sıralandı ve swap işlemi yapıldı, kac kez swap yapıldı sayıldı
"""
#SELECTION SORT
array_1 = [25, 8, 3, 62, 83, 38, 12, 2, 1]
for i in range(len(array_1)):
    for j in range(i + 1, len(array_1)):
        if array_1[i] < array_1[j]:  """ '<' olduğundan küçükten büyüğe"""
            pass
        else:
            temp = array_1[i]
            array_1[i] = array_1[j]
...
p_1 = [4, 0]   # iki nokta verildi distance hesapla
p_2 = [0, 3]
a = (p_1[0] - p_2[0])
b = (p_1[1] - p_2[1])
c = (a ** 2 + b ** 2) ** 0.5
...
def get_distance(p_1, p_2):   # center
    a = p_2[0] - p_1[0]
    b = p_2[1] - p_1[1]
    return (a ** 2 + b ** 2) ** 0.5
...
def myselectionSort(points):
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            if (a > b):
                sayac2 += 1  # degistirme = swap işlemi temp ile başlar sayac2 ile kaç kez değiştirme işlemi yapıldı sayar
                temp = points[i]
                points[i] = points[j]
                points[j] = temp
