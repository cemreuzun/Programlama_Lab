# bir hirsiz bir eve giriyor
# amac, evden max esyayi almak ama cantasinin bir agirligi var
# her esyadan bir tane oldugu ve cantasina en fazla 20 kg alabilecegi varsayılarak en fazla kac liralık kazanc sağlar?
...
for k in range(len(item)-1,-1,-1):	#Sondan başlasin her elemanı denesin.
    agirlik_test = item[k].weight
    toplam_para = item[k].value
    i = k
    while(max_agirlik>=agirlik_test and i>=0):	#Ağırlığı aşana kadar "k" indisindeki eleman ile kaç para kazanabileceğini hesaplasın.
      b = i-1
      if(b==0):
         break
